...
export class StudentService {
  ...
  async create(createStudentDto: CreateStudentDto, image: Express.Multer.File) {
    ...
    if (image) {
      ...
      fs.writeFile(filename, image.buffer, (err: string | any) => {
  ...
  async uploadCertificate(
    ...
    if (image) {
      ...
      return OkResponse('Upload Certificate Successful',HttpStatus.ACCEPTED, {certificate: existingStudent});
    }
    throw new HttpException(
      {
        message: 'Unable to upload Certificate',
        error: 'Unable to upload Certificate',
        errors: ['Unable to upload Certificate', 'ERR_BAD_REQUEST'],
      },
  ...
  async findAll(
    pageSize: number = 25,
    page: number = 0,search: string | any) {
